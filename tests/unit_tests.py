...
class TestSuite(unittest.TestCase):
    ...
    def test_material_information_creation(self):
        ...
        assert (
            properties["density"].reference
            == "C.S. Kim, Thermophysical properties of stainless steels, Argonne National Laboratory, Argonne, Illinois, 1975."
        )
        ...
        assert (
            properties["liquidus_temperature"].reference
            == "C.S. Kim, Thermophysical properties of stainless steels, Argonne National Laboratory, Argonne, Illinois, 1975."
        )
        ...
        path_to_example_data = os.path.join(
            os.path.dirname(__file__), "../examples/AlCu.json"
        )
        mat = mist.core.MaterialInformation(path_to_example_data)
        ...
        assert (
            properties["solidus_eutectic_temperature"].name
        ...
        assert (
            properties["solidus_eutectic_temperature"].reference
            == "O. Sinninger, M. Peters, P.W. Voorhees, Two-Phase Eutectic Growth in Al-Cu and Al-Cu-Ag, Metallurgical and Materials Transactions A, Vol. 49A, pp. 1692-1707, 2018."
        )
        ...
        solute_diffusivities = phase_properties["liquid"].properties[
        ...
        assert (
            abs(
                phase_properties["alpha"].properties["gibbs_thomson_coeff"].value
                - 2.4e-7
            )
            < 1.0e-13
        )
        ...
        assert (
            abs(phase_properties["theta"].properties["solubility_limit"].value - 31.9)
            < 1.0e-13
        )

        # Create from the AlCu_test_in JSON file
        path_to_example_data = os.path.join(
            os.path.dirname(__file__), "AlCu_test_in.json"
        )
        mat = mist.core.MaterialInformation(path_to_example_data)
        assert mat.name == "AlCu"
    ...
    def test_write_additivefoam(self):
        ...
        # Asserts whether the files actually exist
        self.assertTrue(
            os.path.exists(transport_filepath), f"File {thermo_filepath} should exist."
        )
        self.assertTrue(
            os.path.exists(thermo_filepath)
        ), f"File {thermo_filepath} should exist."
            ...
            for i in range(0, len(lines)):
                if i in floating_point_line_numbers:
                    ...
                    try:
                        ...
                        print(
                            "Error: difference exceeded the tolerance",
                            diff,
                            relative_tolerance,
                        )
